...
public class BodySourceView : MonoBehaviour 
{
	...
	//Method takes the joints and connects them with the line renderer, also sets positions of joints
    private void RefreshBodyObject(Kinect.Body body, GameObject bodyObject)
    {
        int updated = 1;
        for (Kinect.JointType jt = Kinect.JointType.HandRight; jt <= Kinect.JointType.HandRight; jt++)
        {
            ...
            if (targetJoint.HasValue)
            {
				...
				if (jt == Kinect.JointType.HandRight){
                    //Debug.Log (GetVector3FromJoint(targetJoint.Value).x + " " + GetVector3FromJoint(targetJoint.Value).y);
                    if (updated == 1)
                    {
                        updated = 0;
                        if (body.TrackingId == bodyOnly)
